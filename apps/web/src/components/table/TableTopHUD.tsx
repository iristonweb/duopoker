...
export function TableTopHUD({
  ...
  return (
    <GlassPanel
      glow="gold"
      className={cn('relative z-30 shrink-0 rounded-none border-x-0 border-t-0 border-gold/25 p-0', className)}
    >
      <div className="border-gradient-gold mx-auto flex h-12 items-center justify-between gap-2 px-3 sm:h-14 sm:gap-3 sm:px-5">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          <h1 className="text-gradient-gold truncate font-display text-sm font-semibold sm:text-base">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          {isJoker ? (
            <div className="hidden md:block">
              <JokerTrumpBadge joker={joker} size="sm" />
            </div>
          ) : null}
          {mode === 'HOLDEM' ? (
          ...
          <div className="glass-shine flex items-center gap-1.5 rounded-full border border-gold/30 bg-white/[0.04] px-2.5 py-1 shadow-glow-gold backdrop-blur-glass sm:gap-2 sm:px-3 sm:py-1.5">
            <PokerChipVisual chipId={chipId} size="sm" />
            <div className="flex flex-col leading-none">
              <span className="text-[8px] font-semibold uppercase tracking-[0.28em] text-gold/70 sm:text-[9px]">
                {potLabel}
              </span>
              <span className="font-mono text-xs font-bold text-gold-light sm:text-sm">
                {typeof potValue === 'number' ? potValue.toLocaleString() : potValue}
              </span>
            </div>
          </div>
          {street && street !== 'LOBBY' && streetLabel ? (
      ...
      <div className="flex justify-center border-t border-white/[0.06] px-3 py-1 sm:hidden">
